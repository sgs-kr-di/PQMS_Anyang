...
namespace userManagement
{
    public partial class MainForm : Form
    {
        ...
        */
        ...
        {
            //login_chk(sUsrID, sUsrPW);
        }

        // 외부 호출용 함수
        public string Call_Login_Chk(string sUsrID, string sUsrPW)
        {
            return login_chk(sUsrID, sUsrPW);
        }
        ...
        private string login_chk(string sUsrID, string sUsrPW)
        {
            accountNo = "";
            userId = "";
            loginId = "";
            ...
            string query = " select accountNo, logInId, userId, appId" +
                         "     from accountMaster with(nolock) " +
                         "    where logInId = '" + sUsrID + "' " +
                         "      and passWord = '" + sUsrPW + "' " +
                         "      and appID = '"+ appId + "' " +
                         "      and requestStatusCode ='approved'" +
                         "      and disuse <> 1 ";
            ...
            using (OleDbDataReader reader = cmd.ExecuteReader())
            {
                if (reader.HasRows == true)
                {
                    while (reader.Read())
                    {
                        ...
                        {
                            if (item.Name == reader.GetValue(0).ToString() + "ToolStripMenuItem" && reader.GetValue(1).ToString() == "False")
                            {
                                item.Visible = false;
                            }
                        }                        
